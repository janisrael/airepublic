...
export default {
  ...
  methods: {
    ...
    async loadUsageStats() {
      try {
        const response = await fetch('http://localhost:5000/api/providers/usage', {
          headers: this.getAuthHeaders()
        })
    ...
    async loadUserConfigs() {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/providers/configs?provider=${this.selectedProvider.name}`, {
          headers: this.getAuthHeaders()
        })
    ...
    async saveProviderConfig() {
      try {
        ...
        const response = await fetch('http://localhost:5000/api/providers/configs', {
          method: 'POST',
          headers: {
            ...this.getAuthHeaders(),
    ...
    async setDefaultConfig(configId) {
      try {
        const response = await fetch(`http://localhost:5000/api/providers/configs/${configId}`, {
          method: 'PUT',
          headers: {
            ...this.getAuthHeaders(),
            'Content-Type': 'application/json'
    ...
    async submitQuery() {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/providers/query/${this.selectedProvider.name}/${this.queryForm.model_id}`, {
          method: 'POST',
          headers: {
            ...this.getAuthHeaders(),
            'Content-Type': 'application/json'
