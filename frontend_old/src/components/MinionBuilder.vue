<template>
  <div class="models-container">
    <div class="page-header">
      ...
      <button class="btn btn-primary" @click="showCreateModelModalV2 = true">
            ...
            <div class="model-info">
              <div class="model-header-with-avatar">
                <div class="model-avatar" v-if="model.avatar_url">
                  <img :src="'http://localhost:5000/' + model.avatar_url" :alt="model.name + ' avatar'" class="avatar-image">
                </div>
...
export default {
  ...
  methods: {
    ...
    getMinionApiUrl(model) {
      ...
      return `http://localhost:5000/api/minions/${model.id}`;
    },
      ...
      try {
        ...
        if (this.selectedChatModel.type === 'external_api') {
          ...
          console.log('External API request:', {
            url: 'http://localhost:5000/api/external-models/chat',
            body: requestBody
          });
          
          const response = await fetch('http://localhost:5000/api/external-models/chat', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
    ...
    async saveModel() {
      ...
      try {
        if (this.modelForm.modelType === 'external') {
          ...
          const response = await fetch('http://localhost:5000/api/external-models', {
            method: 'POST',
            body: formData
          });
    ...
    async fetchLocalModels() {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/users/${authStore.user.id}/minions`, {
          headers: {
    ...
    async fetchReferenceModels() {
      try {
        const response = await fetch('http://localhost:5000/api/v2/reference-models');
        const data = await response.json();
    ...
    async fetchReferenceModelsV2() {
      try {
        const response = await fetch('http://localhost:5001/api/reference-models-v2');
        const data = await response.json();
        
        if (data.success) {
          this.referenceModelsV2 = data.models;
        } else {
    ...
    async saveModelV2() {
      ...
      try {
        if (this.modelFormV2.modelType === 'external') {
          ...
          const response = await fetch(`http://localhost:5001/api/users/${authStore.user.id}/minions`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`
            },
          ...
          const response = await fetch('http://localhost:5001/api/models-v2', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
