...
### 1. External RAG Training (Retrieval-Augmented Generation)
Enhances **external API models** (OpenAI, Anthropic, NVIDIA, etc.) with external knowledge bases using ChromaDB vector storage.

### 2. External LoRA Training (Low-Rank Adaptation)
Fine-tunes **external API models** with custom datasets using LoRA-style adaptation techniques.

### 3. Local LoRA Training (Low-Rank Adaptation)
Fine-tunes **local models** (LLaMA, Mistral, etc.) with custom datasets using traditional LoRA adapters.

### 4. Hybrid Training
Combines RAG and LoRA approaches for comprehensive minion enhancement.

## External RAG Training Process - 9 Steps (0-8)
