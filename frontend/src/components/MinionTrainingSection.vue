...
export default {
  ...
  methods: {
    async fetchMinions() {
      ...
      try {
        ...
        const response = await fetch(
          getUserApiUrl(this.authStore.user.id, 'external-training/jobs?group_by=minion&latest_only=true'),
          {
            headers: {
    ...
    handleViewHistory(minion) {
      console.log('📊 Navigate to history page for minion:', minion)
      // Navigate to dedicated minion history page
      this.$router.push({ name: 'MinionHistoryPage', params: { id: minion.id } })
    },
    ...
    handleStartPendingTraining(job) {
      this.$emit('start-pending-training', job)
        ...
        handleTrainingCompleted(minionId) {
          ...
          this.fetchMinions()
          // Emit event to parent to refresh externalTrainingJobs
          this.$emit('training-completed', minionId)
        },
