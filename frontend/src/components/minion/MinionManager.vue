            ...
            <div class="model-info">
              <div class="model-header-with-avatar">
                <div class="model-avatar" v-if="model.avatar_url">
                  <img :src="getApiUrl(model.avatar_url)" :alt="model.name + ' avatar'" class="avatar-image">
                </div>
              ...
              <div class="model-progress" :style="{ '--progress-width': model.minionData?.experience + '%' || 0 + '%' }">
                <span class="progress-item">
                  <span class="material-icons-round">star</span>
                  Level {{ model.details?.level || 1 }}
                </span>
                <span class="progress-item">
                  <span class="material-icons-round">trending_up</span>
                  {{ model.minionData?.experience || 0 }} %
                  <!-- {{ formatExperience(model.minionData?.experience || 0) }}% -->
                </span>
              </div>
...
import { API_ENDPOINTS, getUserEndpoint, getApiUrl } from '@/config/api'
...
export default {
  ...
  methods: {
    // Parse capabilities from string format to array
    parseCapabilities(capabilities) {
      ...
      if (typeof capabilities === 'string') {
        try {
          ...
          // Handle formats like "{reasoning,planning,problem-solving,analysis,strategic-thinking}"
          const cleaned = capabilities.replace(/[{}]/g, '').trim();
          if (cleaned) {
            return cleaned.split(',').map(cap => cap.trim()).filter(cap => cap.length > 0);
          }
        }
      }
    ...
    async loadModelDetails() {
      ...
      for (const model of this.models) {
        try {
          ...
          const response = await fetch(`API_ENDPOINTS.v1.models + '/' + ${model.name}/details`);
    ...
    formatParameters(parameters) {
      ...
      if (typeof parameters === 'object') {
        const count = parameters.parameters || parameters.param_count || parameters.size || parameters.num_params
        if (count) {
    ...
    formatQuantization(quantization) {
      if (quantization && quantization !== null) return quantization
      
      const params = this.selectedModelForDetails?.details?.parameters || this.selectedModelForDetails?.parameters
      if (params) {
        try {
          const obj = typeof params === 'string' ? JSON.parse(params) : params
          const inferred = obj.quantization || obj.dtype || obj.precision
    ...
    getMinionApiUrl(model) {
      ...
      return `API_ENDPOINTS.v1.minions + '/' + ${model.id}`;
    },
    ...
    async saveModelChanges() {
      try {
        ...
        const response = await fetch(`API_ENDPOINTS.v1.models + '/' + ${encodeURIComponent(this.selectedModelForDetails.name)}`, {
          method: 'PUT',
          headers: {
      ...
      try {
        ...
        if (this.selectedChatModel.type === 'external_api') {
          ...
          console.log('External API request:', {
            url: 'API_ENDPOINTS.v1.externalModels/chat',
            body: requestBody
          });
          
          const response = await fetch('API_ENDPOINTS.v1.externalModels/chat', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
    ...
    async saveModel() {
      ...
      try {
        if (this.modelForm.modelType === 'external') {
          ...
          const response = await fetch('API_ENDPOINTS.v1.externalModels', {
            method: 'POST',
            body: formData
          });
    ...
    async fetchLocalModels() {
      ...
      try {
        ...
        if (data.success) {
          ...
          this.models = data.minions.map((minion, index) => ({
            ...
            capabilities: this.parseCapabilities(minion.capabilities),
    ...
    async fetchReferenceModels() {
      try {
        const response = await fetch('API_ENDPOINTS.v1.referenceModels');
        const data = await response.json();
