...
export default {
    ...
    methods: {
        ...
        initRadarChart() {
            ...
            const option = {
                title: {
                    text: `Performance Radar${this.getDatasetNames(this.item).length > 0 ? `\nDatasets: ${this.getDatasetNames(this.item).join(', ')}` : ''}`,
                    left: 'center',
                    textStyle: {
                        color: '#ffffff',
                        fontSize: 16,
...
.settings-title .material-icons-round {
    font-size: 18px;
    color: var(--primary);
}
